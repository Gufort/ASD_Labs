...
namespace TreeTest
{
	...
	TEST_CLASS(Test_find_and_count_if) {
		template<typename T> using Myal = std::allocator<T>;
		template<typename T> using Mypred = std::less<T>;
		...
		TEST_METHOD(Test_find) {
			ContainerTemplate<int> tree{ 1,2,3,4,6,8,5 };
			Assert::IsTrue(std::find(tree.begin(), tree.end(), 3) == tree.find(3));
			Assert::IsFalse(std::find(tree.begin(), tree.end(), 5) == tree.find(4));
			Assert::IsTrue(std::find(tree.begin(), tree.end(), 8) == tree.find(8));
			Assert::IsTrue(std::find(tree.begin(), tree.end(), 3) == tree.find(3));
		}

		TEST_METHOD(Test_count_if) {
			ContainerTemplate<int> tree{ 1,2,3,6,8,9,4 };
			Assert::IsTrue(std::count_if(tree.begin(), tree.end(), [](int x) { return x % 2 == 0;}) == 4);
			Assert::IsTrue(std::count_if(tree.begin(), tree.end(), [](int x) { return x % 2 != 0; }) == 3);
			Assert::IsTrue(std::count_if(tree.begin(), tree.end(), [](int x) { return x < 5; }) == 4);
			Assert::IsTrue(std::count_if(tree.begin(), tree.end(), [](int x) { return x > 10; }) == 0);
		}
	};
}